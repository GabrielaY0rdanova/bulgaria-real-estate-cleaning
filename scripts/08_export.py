...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Load a validated full rebuild into one explicitly named empty database."
    )
    parser.add_argument(
        "--target-database",
        required=True,
        help="Database name that must match the actual PostgreSQL connection.",
    )
...
if run_context.get("mode") != "full_rebuild":
    raise RuntimeError("08_export.py accepts full_rebuild mode only")
...
try:
    # ------------------------------------------------------------------
    # Step 1: Refuse the wrong database or any non-empty target.
    # ------------------------------------------------------------------
    cursor.execute("SELECT current_database()")
    actual_database = cursor.fetchone()[0]
    if actual_database != args.target_database:
        raise RuntimeError(
            f"Database mismatch: requested {args.target_database!r}, "
    ...
    if listing_count or run_count:
        raise RuntimeError(
            "Full rebuild target is not empty. Create a new replacement database "
            "instead of deleting existing data."
        )
    print(f"  Confirmed empty target database: {actual_database}")
    ...
    # ------------------------------------------------------------------
    # Step 3: Bulk-load every table. The empty-target gate makes upserts
    # unnecessary and avoids hundreds of thousands of row-by-row inserts.
    # ------------------------------------------------------------------
    print("\n--- Loading data ---")
