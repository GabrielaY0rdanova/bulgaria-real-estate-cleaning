...
try:
    # ------------------------------------------------------------------
    # Step 1: Truncate existing data (idempotent re-run support)
    # ------------------------------------------------------------------
    # Note: all DDL in this transaction (ALTER TABLE, TRUNCATE, COPY) is
    # fully transactional in PostgreSQL. On rollback, all changes including
    # schema alterations are undone automatically.
    # ------------------------------------------------------------------
    print("\n--- Truncating existing tables ---")
    truncate_tables(cursor, [name for name, _ in LOAD_ORDER])
    ...
    # ------------------------------------------------------------------
    # Step 3: COPY data into tables (in FK-dependency order)
    # ------------------------------------------------------------------
    print("\n--- Loading data ---")
    ...
    for table_name, _ in LOAD_ORDER:
        ...
        # Validate all expected columns exist in the DataFrame
        ...
        print(f"  ✓ {table_name:<22} {len(df):>8,} rows loaded")
